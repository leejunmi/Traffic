...
class TrafficLightDetector:
    def __init__(self, image):
        ...
        self.green_lower = np.array([35, 135, 135])
    ...
    def find_main_and_left_green(self):
        ...
        # valid_idxs = np.where(areas <= 300)[0]

        # if valid_idxs.size > 0:
        #     best_idx = valid_idxs[np.argmax(areas[valid_idxs])]
        #     max_label = 1 + best_idx
        # else:
        ...
        if thres_lower < r: # and r > y and r > g:
            self.final_decision = 'red'
        elif thres_lower < g  : # and g > r and g > y:
            self.final_decision = 'green'
        elif thres_lower < y : # and y > r and y > g:
        ...
        if self.final_decision == 'red':
            ...
            cv2.rectangle(self.visual_img, (lx1, ly1), (lx2, ly2), (0, 255, 255), 2)
            red_roi1 = self.hsv[ly1:ly2, lx1:lx2]
            try:
                self.red_yellow_mask = cv2.inRange(red_roi1, self.yellow_lower, self.yellow_upper)
            except Exception as e:
                self.red_yellow_mask = np.zeros_like(self.green_mask)

            diff = diff + w + 5
            lx1 = max(x + diff, 0)
            lx2 = min(x + w + diff, self.hsv.shape[1])
            cv2.rectangle(self.visual_img, (lx1, ly1), (lx2, ly2), (0, 255, 255), 2)
            red_roi2 = self.hsv[ly1:ly2, lx1:lx2]
            try:
                self.red_green_mask = cv2.inRange(red_roi2, self.green_lower, self.green_upper)
            except Exception as e:
                self.red_green_mask = np.zeros_like(self.green_mask)
                # print(e)

        # left green ROI
        if self.final_decision == 'green' and g > thres_lower:
            ...
            cv2.rectangle(self.visual_img, (lx1, ly1), (lx2, ly2), (0, 255, 255), 2) # green의 왼쪽 ROI BOX
    ...
    def detect(self):
        ...
        if self.final_decision == 'red':
            ...
            if cv2.countNonZero(self.red_green_mask) > 5: 
                return 'red_and_left_green'
            elif cv2.countNonZero(self.red_yellow_mask) > 5: 
